...
def people():
    ...
    ElectionFactory(position='29', result='1', year=1999, party=party,
                    candidate=jon)

    # jon elected by qupta for federal deputy
    ElectionFactory(position='15', result='2', year=1995, party=party,
                    candidate=jon)

    # tyrion not elected for president
    ElectionFactory(position='29', result='4', year=1999, party=party,
                    candidate=tyrion)
    return jon, tyrion

@pytest.mark.django_db
@pytest.mark.parametrize('position,year,qty,expected', [
...
def test_filter_election_by_position(client, people, position, year, qty,
                                     expected):
    response = client.get(f'/election/position/?position={position}&{year}',
                          follow=True)
