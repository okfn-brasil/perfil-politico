...
class Command(ImportCsvCommand):
    ...
    def get_politician(name):
        ...
        names = tuple(
            Name(obj, get_distance(name, obj.congressperson_name)) for obj in qs
        )
    ...
    def get_deputy(self, line):
        try:
            return Politician.objects.get(
                area="1", congressperson_id=line["congressperson_id"]
            )
    ...
    def get_senator(self, line):
        ...
        try:
            return Politician.objects.get(congressperson_name__unaccent__icontains=name)
