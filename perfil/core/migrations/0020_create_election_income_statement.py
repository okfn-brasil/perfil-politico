# Generated by Django 3.2.5 on 2021-11-27 21:45

from django.db import migrations, models


class Migration(migrations.Migration):

    dependencies = [
        ("core", "0019_precalculatedstats"),
    ]

    operations = [
        migrations.CreateModel(
            name="ElectionIncomeStatement",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("date", models.DateField(null=True)),
                ("year", models.IntegerField()),
                ("value", models.DecimalField(decimal_places=2, max_digits=16)),
                (
                    "document_number",
                    models.CharField(blank=True, default="", max_length=100),
                ),
                (
                    "receipt_number",
                    models.CharField(blank=True, default="", max_length=75),
                ),
                (
                    "description",
                    models.CharField(blank=True, default="", max_length=255),
                ),
                (
                    "accountant_sequential",
                    models.CharField(blank=True, default="", max_length=16),
                ),
                (
                    "accountant_taxpayer_id",
                    models.CharField(blank=True, default="", max_length=11),
                ),
                (
                    "vice_candidate_taxpayer_id",
                    models.CharField(blank=True, default="", max_length=11),
                ),
                (
                    "deputy_substitute_taxpayer_id",
                    models.CharField(blank=True, default="", max_length=11),
                ),
                (
                    "donor_name",
                    models.CharField(blank=True, default="", max_length=256),
                ),
                (
                    "donor_taxpayer_id",
                    models.CharField(blank=True, default="", max_length=16),
                ),
                (
                    "donor_economic_sector_code",
                    models.CharField(blank=True, default="", max_length=10),
                ),
                ("additional_income_information", models.JSONField(default=dict)),
                ("donor_company_information", models.JSONField(default=dict)),
            ],
            options={
                "verbose_name": "election income statement",
                "verbose_name_plural": "election income statements",
                "db_table": "core_election_income_statement",
                "ordering": ("accountant_sequential", "year"),
            },
        ),
        migrations.AddIndex(
            model_name="electionincomestatement",
            index=models.Index(
                fields=["accountant_sequential"], name="core_electi_account_620c69_idx"
            ),
        ),
        migrations.AddIndex(
            model_name="electionincomestatement",
            index=models.Index(
                fields=["accountant_taxpayer_id"], name="core_electi_account_714991_idx"
            ),
        ),
    ]
